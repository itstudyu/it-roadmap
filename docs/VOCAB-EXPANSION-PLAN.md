...
`tools/build.py` 의 `slugify()` 를 그대로 불러서 후보의 id 를 미리 계산하고
...
Churn Rate · DAU MAU · Cohort Analysis · Conversion Rate · NPS ·
Value Proposition · PRD · Usability Test · Kanban · Retrospective ·
Definition of Done · Story Point · Stakeholder · Scope Creep · Competitive Analysis

> RICE · Jobs to be Done · Design Thinking 셋은 **버렸다.** 원전이 각각 Intercom
> 블로그, 단행본·HBR 기고, IDEO 교육 자료라 1차 출처를 세울 수 없다.
> 없는 근거를 만들어 넣는 것보다 세 편을 비워 두는 편이 낫다.
...
230편 × 2 = **460건을 실제로 열어서 읽어야 한다.** 이게 이 작업 비용의 대부분이다.
...
이유다. 루틴 속도(주 2편)로는 230편에 **2년 3개월**이 걸린다.
...
- **(나) 제품 관리를 이번 계획에서 뺀다.** 230 → 215편. 나머지 11권만 한다.
...
해소돼서 후보가 0이다. 230편이 새 링크를 만들면 **후보 풀이 생긴다.** 이건 고칠 게 아니라
예상되는 결과이고, 좋은 쪽이다. 루틴 재개 뒤 첫 금요일에 확인할 것.

### 5.3 `content/INDEX.md` 는 이미 낡았다

"총 **238개**" 라고 적혀 있는데 실제는 229편이다. 이번에 459편이 되면 더 벌어진다.
...
**한 번에 230편을 커밋하지 않는다.** 권 단위 배치로 자르고, 배치마다 검증 → 커밋한다.
...
| **slug 충돌** | 새 단어가 남의 id 를 빼앗아 진도가 엉뚱한 단어에 붙는다 | `build.py` 의 `slugify()` 로 미리 계산 (230편 충돌 0건) |
...
| **`index.js` 가 커진다** | 첫 화면에서 통째로 읽는다. 37KB → 약 75KB (gzip) | 459편까지는 감당한다. 그 이상이면 색인을 권별로 쪼갠다 |
...
1. ~~어디까지 갈 것인가~~ → **전부.** 파도 단위로 자르고 파도마다 검증·커밋한다.
2. ~~제품 관리~~ → **(가).** 방법론 원전을 허용 목록에 넣어 15편을 살리고 3편을 버렸다.
3. ~~허용 목록~~ → **추가함.** `dac622c` 커밋.

---

## 10. 끝난 뒤 (2026-08-23)

계획은 459편이었는데 **629편**에서 끝났다. 도중에 요구가 둘 늘었기 때문이다 —
인프라·네트워크·데이터에서 전문가가 될 것, 그리고 클라우드·솔루션 아키텍트
자격증에 필요한 지식 전부.

```
단어 629편 (계획 459 + 전문가 86 + 자격증 97 − 겹침)
퀴즈 재료  오해 1,887항목 · 사례 1,887항목 · 도해 629편
data/index.js 435KB (gzip 114KB, 첫 화면에서 통째로 읽는다)
data/terms/  5.2MB (권별로 나눠 필요할 때 읽는다)
각본 629편 — 모든 편이 그림을 갖는다
```
...
기울기가 뒤집혔다. 출발할 때 가장 얇던 인프라(13편)가 이제 둘째로 두껍다.
의도한 것이다 — 이 셋에서 전문가가 되겠다는 것이 요구였다.

### 완료 기준 대조

- [x] 기존 id **전부 무변경** — 파도마다 기계로 쟀다. 사라진 id 0 · 이름 바뀐 id 0
- [x] `check_template.py` 새 파일 전부 `--require=examples,aim,loop,even` 통과
- [x] 편당 서로 다른 도메인 1차 출처 2건 이상
- [x] `INDEX.md` 자동 생성으로 전환 → `tools/build_index.py`
- [x] 배포 후 실서버가 629편을 물고 뜨는 것 확인
- [ ] `ROUTINE-PROMPT.md` 상한 — **고치지 않기로 했다.** 5권이 60을 넘었지만
      7권이 아직 밑이라 루틴은 자연히 얇은 권으로 간다. 상한의 목적이
      "한 권만 부풀지 않게" 인데, 지금 그 목적대로 작동하고 있다
- [ ] `sources.allowlist.md` 갱신 — 자격증 조사에서 새로 쓴 도메인 없음

### 남은 것 하나

**첫 화면이 114KB(gzip)를 읽는다.** 이 계획서가 세워질 때 37KB 였다. 안에서
`related[].note` 가 26%를 차지하는데, 이걸 권별 본문 파일로 내리면 첫
읽기가 눈에 띄게 준다. 다만 `js/quiz.js:585` 가 전체 풀에서 `answer.note`
를 쓰고 `js/screens.js:748` 이 그린다. 데이터 모양을 바꾸는 일이라 단어를
더하는 것과 같은 파도에 태우면 안 된다. **다음 파도에서 따로 한다.**

### 이번에 배운 것

집필자에게 검수를 맡기면 안 된다. 자기가 안 연 URL 을 적었는지는 자기가
못 잡는다. 묶음마다 **독립 검수자**를 붙여 각주 URL 을 전부 다시 열게 했더니
매 묶음에서 거짓이 나왔다 — RFC 4786 에 없는 루프백 서술, 자바독에 없는
인용문, EC2 문서에 없는 숫자, 인용한 페이지에 없는 대역폭 범위.

그리고 **조용히 아무것도 안 하는 것이 가장 나쁜 실패다.** `reviewOnly` 로
집필을 건너뛰려고 `null` 을 돌려줬더니 pipeline 이 그 항목을 통째로
떨어뜨려 검수까지 건너뛰었다. 에러 없이, 에이전트 0개로 "완료" 라고
보고하면서. 그 사이 검수를 못 받은 편에는 H1 이 틀린 것까지 있었다.
