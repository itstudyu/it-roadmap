...
### 이름
Lambda :: 그리스 글자 λ
= 함수를 λ 하나로 적던 옛 수학 표기에서 왔다. 빌리는 단위가 서버가 아니라 함수라는 뜻이다

### 비유
택시. 필요할 때 부르고 탄 만큼만 낸다. 자가용은 세워두기만 해도 보험과 주차비가 나간다.

### 예
사진을 올린 뒤 목록에 작게 줄어든 그림이 뜨기까지 몇 초 걸리는데, 그 사이가 이런 함수가 일하는 자리다.
...
```도해
층: Lambda 에 올린 함수는 무엇 위에 얹혀 도나
함수 코드 :: 내가 올리는 것. 사건을 받아 처리한다
런타임 :: 그 언어를 해석해 코드를 돌려주는 환경
실행 환경 :: 런타임과 코드를 함께 담아 띄우는 상자
AWS 서버 :: 그 상자가 실제로 올라가는 기계
= 내가 올리는 것은 맨 위 한 층뿐이고 그 아래는 AWS 가 세워둔다
```
...
**"부를 때만 깨어나는 그 함수 한 조각"** 이라고 읽으면 된다. 노는 시간에 돈이 나가지 않고 몰릴 때 알아서 늘어나는 대신, 첫 호출이 느리고 실행 시간과 크기에 제한이 있다. 뜸하게 불리는 짧은 작업에 맞고 쉬지 않고 도는 작업에는 맞지 않는다.

## 🧒 열 살에게

택시는 탈 때만 부르고 탄 만큼만 돈을 내지? 우리 집 차는 세워만 놔도 주차비가 계속 나가는데 말이야. 대신 오랜만에 부르면 택시가 올 때까지 한참 기다려야 하고, 날마다 아침저녁으로 탈 거면 차를 한 대 두는 편이 싸.
