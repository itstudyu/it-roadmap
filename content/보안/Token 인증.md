...
### 이름
Token :: 자격을 담은 표
Authentication :: 본인 맞는지 확인
= 붙여 읽으면 "표로 본인을 확인함". 표 한 장이 아이디와 비밀번호를 대신한다
...
**"입구에서 받은 팔찌만 보이면 되는 그 방식"** 이라고 읽으면 된다. 서버가 아무것도 기억하지 않아도 되므로 서버를 늘리기 쉽고 웹과 모바일이 같은 방식을 쓴다. 대신 이미 나간 토큰은 만료 전까지 살아 있어서 즉시 끊는 일이 어렵다.

## 🧒 열 살에게

놀이공원 입구에서 표를 내면 손목에 종이 팔찌를 채워 주지? 그다음부터는 놀이기구마다 표를 다시 내지 않고 팔찌만 보여 주면 돼. 팔찌에는 몇 시까지인지가 적혀 있어서 시간이 지나면 못 쓰지만, 그 전에 남이 주워 차면 문지기는 그냥 들여보내 줘.
