...
Interface :: 창구
Key :: 열쇠
= 붙여 읽으면 "프로그램을 불러 쓰는 창구의 열쇠". 열쇠마다 주인 이름이 새겨져 있다

### 비유
헬스장 회원카드. 출입구에 찍으면 누구인지, 어떤 회원권인지, 오늘 몇 번째 방문인지가 한 번에 확인된다.

### 예
가게 홈페이지의 지도가 회색으로 덮이고 오류 문구만 뜬 것을 본 적이 있다. 그 사이트가 쓰던 지도 키가 끊긴 것이다.

### 직접
지도가 들어간 가게 홈페이지에서 F12 로 Network 탭을 열고 새로고침해 보라. 주소에 `key=` 나 `appkey=` 가 붙은 요청이 있고, 그 뒤 문자열이 그 가게가 쓰는 지도 API Key 다.
...
서버는 발급한 Key를 그대로 저장하지 않고 해시(되돌릴 수 없는 지문 값)로 바꿔 저장한다. 요청이 오면 받은 Key를 같은 방식으로 해시해서 대조한다. 저장소가 유출돼도 원본 Key는 나오지 않는다.
...
**"부르는 쪽에 이름표를 붙이는 그 문자열"** 이라고 읽으면 된다. 그 이름표가 있어야 권한을 나누고 한도를 걸고 사용량을 청구할 수 있다. 다만 내용을 감추지는 못하므로 HTTPS와 함께 쓰고, 새면 지우는 게 아니라 다시 발급한다.

## 🧒 열 살에게

헬스장 갈 때 회원카드를 찍지? 그 카드에 네 이름이 붙어 있어서 오늘 몇 번 들어왔는지가 세어지고, 수영장까지 들어가도 되는지 안 되는지도 정해져. 카드를 잃어버리면 그 카드 하나만 못 쓰게 막고 새 카드를 주는 것도 똑같아.

## ❓ 이해했는지

- 발급 화면에서 Key 원문을 한 번밖에 못 보는 이유는 → 작동 원리
- Key를 붙였는데도 중간에서 요청 내용이 읽힌다면 무엇이 빠진 것인가 → 흔한 오해
