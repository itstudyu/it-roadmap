...
Secure :: 엿볼 수 없는
Shell :: 명령을 주고받는 껍데기
= 붙여 읽으면 "엿볼 수 없는 명령 창". 멀리 있는 컴퓨터의 명령 창을 가린 통로 안에서 연다

### 비유
집 열쇠와 자물쇠. 자물쇠는 집 문에 걸어두고 열쇠는 주머니에서 꺼내지 않는다. 자물쇠만 보고는 열쇠를 깎을 수 없다.

### 예
서버에 처음 붙을 때 이 호스트를 믿겠냐고 묻고 yes 를 치게 하는 그 물음이 이 방식의 첫 화면이다.

### 직접
명령 창을 열고 `ssh` 만 쳐 보라. 쓰는 법이 주르륵 나오면 이 컴퓨터에도 이미 들어 있는 것이다.
...
**"멀리 있는 컴퓨터에 가린 통로로 들어가는 그 약속"** 이라고 읽으면 된다. 기본 포트는 22 이고, 개인키는 내 컴퓨터에 두고 공개키만 서버에 등록해 서명으로 신원을 확인하기 때문에 비밀번호를 주고받지 않는다. 통로 하나 위에 셸, 파일 전송, 포트 포워딩이 함께 올라간다.

## 🧒 열 살에게

멀리 있는 내 방 문에 자물쇠를 걸어 두고, 그 열쇠는 주머니에서 절대 안 꺼내. 문 앞에 가면 "진짜 주인 맞아?" 하고 묻는데, 열쇠를 보여 주는 게 아니라 열쇠로 도장만 꾹 찍어서 보여 줘. 그래서 오가는 길에 누가 몰래 엿봐도 열쇠는 못 얻고, 방 안에서 무슨 말을 했는지도 못 봐.
