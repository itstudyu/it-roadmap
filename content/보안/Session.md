...
### 이름
Session :: 앉아 있는 한 동안
= 라틴말 "앉음" 에서 왔다. 들어와 앉았다가 일어설 때까지가 한 세션이다
...
### 직접
브라우저에서 F12 를 눌러 Application 탭의 Cookies 를 열어 보라. 로그인해 둔 사이트에 이름에 session 이 들어간 값 하나가 보인다.
...
**"손님은 번호표만 들고 장부는 가게가 쥐는 그 방식"** 이라고 읽으면 된다. HTTP가 이전 요청을 기억하지 않는다는 점을, 서버가 정보를 들고 브라우저에는 그 자리 번호만 주는 것으로 메운다. 번호가 곧 신분증이라 쿠키 설정과 만료 시간이 안전한지 아닌지를 거의 정한다.

## 🧒 열 살에게

단골 분식집에서 회원카드를 한 장 주지? 카드에 적힌 건 번호 하나뿐이고, 네가 뭘 좋아하는지는 가게 공책에 적혀 있어. 그래서 카드를 잃어버리면 주운 사람이 네 행세를 할 수 있고, 한참 안 가면 아줌마가 공책에서 네 줄을 지워 버려.
