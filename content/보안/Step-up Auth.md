...
Step-up :: 한 칸 올림
Auth :: 본인 맞는지 확인
= 붙여 읽으면 "확인 수준을 한 칸 올림". 문을 새로 다는 게 아니라 이미 연 문 앞에서 한 칸 올린다

### 비유
건물 출입증과 금고실 지문 인식. 출입증으로 건물에는 들어가지만 금고실 문 앞에서는 한 번 더 확인한다.

### 예
쇼핑 앱에서 담고 구경하는 것은 그냥 되는데 결제 버튼을 누르는 순간 여섯 자리 비밀번호를 묻는다.
...
@ 다시 송금으로 :: 이번에는 Level 2 라 그대로 처리된다
...
| 유효 기간 | 길다 | 5분에서 10분 |
| 보호 대상 | 앱 전체 접근 | 특정 작업 |
| 확인 방법 | 아이디와 비밀번호 | 비밀번호, OTP(일회용 코드), 생체 |
...
**"금고실 문 앞에서만 한 번 더 묻는 그 절차"** 라고 읽으면 된다. 확인이 끝나면 수준이 오른 토큰을 짧은 시간만 주고 그 시간이 지나면 저절로 내려간다. 로그인 한 번이 모든 작업의 허가가 되지 않게 나누는 것이 목적이다.

## 🧒 열 살에게

회사 건물에 들어갈 때는 목에 건 카드만 문에 대면 되지? 그런데 돈이 든 방 앞에서는 카드를 대도 안 열리고 손가락을 대야 열려. 그것도 딱 십 분만 열려 있어서, 자리를 비운 사이 누가 네 카드를 집어 가도 그 방만은 못 들어가.

## ❓ 이해했는지

- 로그인된 폰을 남이 집었을 때 잔액은 보이는데 송금은 막히는 이유는 → 해결하는 문제
- 송금을 한 번 하고 3분 뒤에 또 하면 비밀번호를 다시 묻지 않는 이유는 → 실제 사례
- 승격 시간을 길게 잡으면 무엇을 잃나 → 작동 원리
