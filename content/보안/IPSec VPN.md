...
### 이름
Internet Protocol :: 주소 붙여 보내는 약속
Security :: 잠그고 확인함
Virtual :: 진짜는 아닌
Private :: 우리끼리 쓰는
Network :: 이어 놓은 망
= 붙여 읽으면 "주소 약속을 잠가 만든, 우리끼리 쓰는 것처럼 보이는 망". 회선이 새로 깔리는 게 아니라 그렇게 보이게 하는 것이다

### 비유
투명 봉투 대신 잠긴 상자에 넣어 보내는 것. 배달하는 사람은 상자를 옮길 뿐 안을 보지 못한다.

### 예
지사 사무실 컴퓨터가 전용선 없이도 본사 서버를 사내 주소 그대로 여는 것이 두 망을 이 방식으로 묶어 둔 결과다.
...
**"공용 길 위에 잠긴 통로를 낸 그 방식"** 이라고 읽으면 된다. 암호화, 변조 검사, 상대 확인을 아래층에서 처리해서 위에 있는 프로그램은 손댈 게 없다. 지켜주는 구간은 터널의 양 끝 사이뿐이다.

## 🧒 열 살에게

택배를 투명한 비닐에 담아 보내면 옮기는 사람이 안을 다 볼 수 있지? 그래서 자물쇠 달린 상자에 넣어 보내는 거야. 길은 여전히 남들도 다니는 그 길이라, 길이 막히는 날에는 잠긴 상자도 똑같이 늦게 도착해.

## ❓ 이해했는지

- 카페 와이파이에서 VPN을 켰는데도 위험이 남는 구간은 어디인가 → 흔한 오해
- IPSec을 쓰면 브라우저나 메일 프로그램을 고치지 않아도 되는 이유는 → 구성
