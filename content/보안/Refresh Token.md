...
Refresh :: 새것으로 바꿈
Token :: 자격을 담은 표
= 붙여 읽으면 "표를 새로 받아 오는 표". 이 표로는 문을 못 열고, 문 여는 표를 다시 받아 올 뿐이다

### 비유
회원증과 하루짜리 락커 열쇠. 열쇠는 매일 반납하지만 회원증을 내밀면 새 열쇠를 다시 받는다.

### 예
쇼핑 앱은 몇 주 만에 열어도 로그인이 그대로인데 은행 앱은 며칠만 지나도 다시 로그인하라고 한다. 그 차이가 여기서 갈린다.
...
- **리프레시 토큰을 `localStorage`에 두지 않는다.** [[XSS]]가 한 번 통하면 30일짜리 열쇠가 통째로 나간다.
- **로그아웃에서 서버 쪽 무효화를 빠뜨리지 않는다.** 클라이언트에서 지우기만 하면 이미 복사된 토큰은 만료일까지 살아 있다.

## 📝 정리

**"짧은 열쇠를 다시 받아 오는 그 회원증"** 이라고 읽으면 된다. 자주 나다니는 토큰과 오래 사는 토큰을 갈라놓아, 액세스 토큰이 만료되면 이것으로 새로 받아 사용자가 다시 로그인하지 않게 한다. 대신 이 토큰 자체는 잘 내보내지 않고 어디에 두는지도 따진다.

## 🧒 열 살에게

수영장에 가면 사물함 열쇠는 그날 저녁에 반납해야 하지? 대신 회원증은 지갑에 오래 넣어 두고, 다음에 가서 그걸 내밀면 새 열쇠를 또 줘. 그래서 열쇠를 잃어버려도 그날이면 못 쓰게 되지만, 회원증을 잃어버리면 남이 계속 새 열쇠를 받아 가.

## ❓ 이해했는지

- 액세스 토큰이 만료됐는데도 사용자가 다시 로그인하지 않아도 되는 이유는 → 그림
- 액세스 토큰 수명을 30일로 잡으면 무엇이 위험해지나 → 해결하는 문제
- 이미 교체한 옛 리프레시 토큰이 다시 들어왔을 때 서버가 계보를 전부 끊는 이유는 → 작동 원리
...
- [[XSS]] — `localStorage` 에 둔 토큰을 그대로 읽어가는 공격
