...
Access :: 들어가 쓰기
Control :: 되고 안 되고를 가름
= 붙여 읽으면 "맡은 자리를 근거로 출입을 가른다". 사람이 아니라 자리를 본다
...
### 직접
구글 드라이브에서 아무 파일이나 골라 '공유' 를 눌러 보라. 사람 이름 옆에 뷰어·댓글 작성자·편집자 중 하나를 고르게 되어 있고, 그 자리를 바꾸면 그 사람이 할 수 있는 일이 통째로 바뀐다.
...
**"사람 말고 자리에 권한을 붙이는 그 방식"** 이라고 읽으면 된다. 사람은 자주 바뀌고 역할은 잘 안 바뀌므로 안 바뀌는 쪽에 권한을 붙인다. 대상이 누구 것인지 따지는 조건은 역할 밖에서 따로 확인해야 한다.

## 🧒 열 살에게

학교에서 도서부가 되면 도서실 열쇠를 받고, 그만두면 그 열쇠를 다음 도서부한테 넘겨주지? 열쇠를 네 이름에 붙이는 게 아니라 "도서부" 라는 자리에 붙여 둔 거야. 그래서 사람이 아무리 바뀌어도 열쇠를 새로 깎을 일이 없어.
