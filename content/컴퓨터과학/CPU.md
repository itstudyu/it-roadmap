...
Processing :: 처리하는
Unit :: 장치
= 붙여 읽으면 "한가운데서 처리하는 장치". 계산은 결국 이 한 곳을 거친다

### 비유
요리사. 주문표를 한 장씩 집어 읽고 그대로 만든다. 요리사가 여럿이면 주문을 나눠 맡을 수 있다.

### 예
무거운 프로그램을 켜면 노트북이 뜨거워지고 팬이 크게 도는 그 순간이다.

### 직접
큰 파일을 압축하면서 코어별 그래프를 열어 보라 — 윈도우는 작업 관리자 그래프에 오른쪽 단추로 "논리 프로세서", 맥은 활성 상태 보기의 "창 → CPU 사용 기록" 이다. 여덟 칸 중 한 칸만 치솟는다.
...
**"명령을 한 장씩 집어 해치우는 그 부품"** 이라고 읽으면 된다. 하나를 더 빠르게 만드는 길이 막혀 코어를 여러 개 두는 쪽으로 갔고, 그래서 프로그램이 일을 나눠 짜야 그 개수가 쓸모를 갖는다.

## 🧒 열 살에게

주방의 요리사는 주문표를 한 장씩 집어서 적힌 대로 만들지? 컴퓨터 안에도 그런 요리사가 있어서, 하라고 적어 준 것을 위에서부터 한 줄씩 집어 해치워. 요리사를 여덟 명으로 늘려도 주문을 나눠 적어 주지 않으면 한 명만 일하고 일곱은 그냥 서 있어.

## ❓ 이해했는지

- 3GHz 구형 CPU보다 2GHz 최신 CPU가 빠를 수 있는 이유는 → 흔한 오해
- 8코어 컴퓨터인데 프로그램 실행 시간이 그대로라면 무엇이 빠진 것인가 → 해결하는 문제
- CPU가 메모리까지 나가지 않고 값을 찾으면 무엇이 달라지나 → 작동 원리
