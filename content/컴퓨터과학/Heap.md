...
### 이름
= 쌓아 올린 무더기를 뜻하는 말. 가지런히 줄 세운 게 아니라 쌓아만 둔다는 뜻이 그대로 남았다

### 비유
피라미드 모양의 대기 줄. 맨 위 한 명만 바로 꺼낼 수 있고, 빈 자리는 아래에서 올라와 메운다.

### 예
병원 접수에서 온 순서와 상관없이 급한 환자가 먼저 불려 들어가는 그 줄이다.
...
**"꼭대기 하나만 맞춰 두는 더미"** 라고 읽으면 된다. 전체를 정렬하지 않고 꼭대기만 맞추기 때문에 우선순위 큐를 만드는 데 쓴다. 메모리 힙은 이름만 같은 다른 개념으로, 크기를 미리 모르는 데이터를 두는 영역이다.

## 🧒 열 살에게

병원에서 접수한 순서대로 부르지 않고 급한 사람을 먼저 부르는 걸 본 적 있어? 이건 줄 전체를 급한 순서대로 다시 세우는 게 아니라, 맨 앞 한 명만 늘 제일 급한 사람이 되게 해 두는 거야. 그래서 새 사람이 와도 줄을 통째로 흔들지 않고 위쪽 몇 자리만 슬쩍 바꾸면 돼.

## ❓ 이해했는지

- 힙을 그대로 출력했더니 `[1, 3, 2, 5]` 처럼 뒤섞여 보인다. 왜 그런가 → 흔한 오해
- 작업 100만 개 중 가장 급한 것만 계속 꺼내야 한다. 왜 매번 정렬하지 않나 → 해결하는 문제
- 함수가 끝난 뒤에도 남아 있어야 하는 큰 데이터는 어느 영역에 두나 → 비교

## 🔗 관련 용어

- [[Stack]] — 함수가 쓰고 반납하는 메모리 영역. 힙과 짝을 이룬다
- [[Queue]] — 들어온 순서로 꺼내는 쪽. 힙은 우선순위로 꺼낸다
- [[자료구조]] — 힙이 속한 갈래
- [[Memory]] — 힙이 놓이는 자리
- [[Big O 표기법]] — 넣고 꺼내는 비용을 재는 잣대
