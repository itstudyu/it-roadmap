...
Con- :: 함께
-current :: 흐르는
= 붙여 읽으면 "함께 흐름". 같은 시각에 도는 게 아니라 같은 기간에 함께 나아간다는 뜻이다
...
### 직접
큰 파일 두세 개를 연달아 눌러 내려받아 보라. 막대가 나란히 차오르고, 앞의 것이 끝나기를 기다리지 않는다.
...
**"기다리는 틈에 다른 일을 끼워 넣는 방식"** 이라고 읽으면 된다. 대기가 많은 일에서 큰 차이를 내지만 계산량 자체는 줄여주지 않고, 자리를 비켜주지 않는 계산 코드가 끼면 그 지점에서 전부 멈춘다.

## 🧒 열 살에게

식당에 직원이 한 명뿐인데도 손님 여럿이 같이 밥을 먹지? 주문을 주방에 넣어 놓고 음식 나올 때까지 서 있는 게 아니라, 그 사이에 다음 손님 주문을 받으러 가거든. 그래서 세 사람이 기다리는 시간이 겹쳐지고, 다 같이 훨씬 빨리 먹게 돼.
