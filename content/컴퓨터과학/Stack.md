...
### 이름
= 본래 "차곡차곡 쌓아 올린 더미" 다. 위에만 얹고 위에서만 집는 그 더미가 이름이 됐다
...
### 직접
인터넷 창에서 뒤로 가기 단추를 꾹 눌러 보라. 방금 본 쪽부터 거꾸로 쌓인 목록이 펼쳐진다.
...
**"마지막에 얹은 것이 먼저 나오는 더미"** 라고 읽으면 된다. 넣기와 꺼내기가 모두 O(1)이고, 함수 호출과 되돌리기처럼 역순이 필요한 곳에 맞는다.

## 🧒 열 살에게

씻은 접시를 위에 척척 얹어 두면 꺼낼 때도 맨 위 것부터 집게 되지? 맨 아래 접시가 필요하면 위에 얹은 걸 전부 내려놔야 해. 그래서 마지막에 올린 게 늘 제일 먼저 내려와.
