...
### 이름
= 부두 노동자(docker)에서 왔다. 규격 상자를 배에 싣고 내리는 사람, 안에 뭐가 들었는지는 묻지 않는다
...
**"환경째 상자에 담아 그대로 옮기는 그 도구"** 라고 읽으면 된다. 환경을 문서가 아니라 실행 가능한 파일로 만든 것이 요점이고, 데이터는 밖에 둬야 한다.

## 🧒 열 살에게

이사 갈 때 짐을 손에 들고 하나씩 나르면 뭐가 빠졌는지도 모르지? 그래서 쓰던 걸 큰 상자에 통째로 담아 옮기면, 새 집에서 상자를 열었을 때 원래 방이 그대로 나와. 컴퓨터 일도 그렇게 상자째 옮기니까 남의 컴퓨터에서도 똑같이 굴러가는 거야.
