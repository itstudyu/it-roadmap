...
Pull :: 당겨 가다
Request :: 청하다
= 붙여 읽으면 "내 것을 당겨 가 달라는 청". 명령이 아니라 청이라서, 받는 쪽이 읽고 정한다

### 비유
출판사에 넘기는 원고. 바로 책에 실리는 게 아니라, 편집자가 읽고 고칠 곳을 적어 준 뒤에 실린다.

### 예
GitHub 저장소 위쪽 Pull requests 탭에 숫자와 함께 쌓여 있는 그 목록 하나하나가 이것이다.
...
```도해
층: 풀 리퀘스트는 무엇 위에 얹혀 있나
가지와 커밋 :: git 이 맡는 자리. GitHub 없이도 있다
임시 참조 :: 올린 갈래와 합친 결과를 GitHub 이 잡아 둔다
차이 화면 :: 그 참조를 읽어 바뀐 줄을 펼쳐 보인다
리뷰와 승인 :: 사람이 그 화면을 읽고 합쳐도 되는지 판정한다
= 맨 아래만 git 이 하는 일이고, 그 위는 전부 GitHub 이 얹은 것이다
```
...
**"합치기 전에 읽어 달라고 내미는 그 청"** 이라고 읽으면 된다. git 은 가지와 병합만 알고, 그 위에 차이 화면과 리뷰와 승인을 얹은 것은 GitHub 이다. 그래서 PR 을 잘 쓰는 일은 명령을 외우는 일이 아니라 팀이 무엇을 기대할지 합의하는 일이다.

## 🧒 열 살에게

글을 써서 출판사에 보내면 바로 책이 되는 게 아니라 편집자가 먼저 읽고 고칠 데를 적어 주지? 여럿이 같이 만드는 것도 그래서, 내가 고친 걸 곧장 밀어 넣지 않고 봐 달라고 내밀어. 읽어 본 사람이 좋다고 해야 들어가니까 이상한 게 슬쩍 끼어들기 어려워.
