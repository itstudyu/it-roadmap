...
Java :: 당시 인기 있던 다른 언어
Script :: 미리 적어 둔 대본
= 붙여 읽으면 "인기 언어 이름을 빌린 대본". Java 쪽은 마케팅이고, 정식 규격 이름은 ECMAScript 다
...
### 직접
아무 페이지에서 개발자 도구의 콘솔을 열고 `document.title = "안녕"` 을 쳐 보라. 문서를 다시 받지 않았는데 탭 이름이 바뀐다.
...
**"받아 둔 문서를 그 자리에서 고치는 그 언어"** 라고 읽으면 된다. 문법과 내장 객체는 ECMA-262 규격이 정하고, 화면과 네트워크를 만질 힘은 브라우저가 호스트 환경으로 얹어 준다. 그래서 같은 문법이 브라우저 밖에서도 도는데 `document` 는 브라우저 안에서만 있다.

## 🧒 열 살에게

방에 벽지랑 가구가 다 있어도 스위치가 없으면 불은 안 켜지지? 인터넷 화면도 똑같아. 글자를 치는 동안 밑에 추천 낱말이 스르륵 따라 내려오는 건, 화면을 통째로 새로 받지 않고 그 자리만 고쳐 주는 스위치가 붙어 있어서야.
