...
Object :: 짝지어 묶은 덩어리
Notation :: 적는 법
= 붙여 읽으면 "그 언어의 덩어리를 적는 법". 태어난 곳만 이름에 남았고 지금은 어디서나 쓴다

### 비유
택배 상자에 붙이는 내용물 목록. 사람도 읽을 수 있고, 받는 쪽 기계도 같은 종이를 그대로 읽어 푼다.

### 예
앱에서 목록을 새로 고칠 때 잠깐 도는 동그라미 뒤로 서버가 보내오는 것이 대개 이 글자 뭉치다.

### 직접
크롬에서 F12 로 Network 탭을 켜고 위쪽 `Fetch/XHR` 을 고른 뒤 새로 고쳐 보라. 목록의 항목 하나를 눌러 Response 를 열면 중괄호로 시작하는 그 글자 뭉치가 그대로 보인다.
...
```도해
층: JSON 한 덩어리는 무엇이 무엇을 품고 있나
객체 :: 중괄호로 묶은 짝들의 자리
짝 :: 큰따옴표를 두른 이름과 값 하나를 콜론으로 이은 것
값 :: 글자 숫자 참거짓 널. 배열이나 객체가 다시 올 수 있다
= 값 자리에 객체가 또 들어가므로 이 세 겹이 얼마든지 깊어진다
```
...
**"이름과 값을 짝지어 적은 그 글자 목록"** 이라고 읽으면 된다. 타입이 여섯 가지뿐이고 문법이 짧아서 거의 모든 언어가 그대로 읽고 쓴다. 오가는 동안에는 객체가 아니라 글자라는 점이 모든 규칙의 이유다.

## 🧒 열 살에게

택배 상자에 뭐가 들었는지 적어 붙인 종이 봤지? 물건 이름이랑 개수를 나란히 적고, 안에 작은 상자가 또 있으면 그 안까지 줄을 나눠 적어. 보내는 사람과 받는 사람이 쓰는 말이 달라도 그 한 장만 보면 상자 안을 똑같이 알아.

## ❓ 이해했는지

- 설정 파일의 키를 작은따옴표로 감쌌더니 프로그램이 뜨지 않는다. 왜 그런가 → 주의사항
- 파이썬에서 만든 값을 자바스크립트가 그대로 읽을 수 있는 이유는 → 해결하는 문제
- 응답에 담긴 날짜가 받는 쪽에서 문자열로 나온다. 왜 그런가 → 흔한 오해
