...
Data :: 쌓아 둔 자료
Base :: 두는 자리
= 붙여 읽으면 "자료를 두는 자리". 두기만 하는 게 아니라 어긋나지 않게 지킨다

### 비유
도서관. 책을 아무 데나 쌓아두는 게 아니라 분류하고 색인을 만들어두어서, 누가 무엇을 언제 빌려갔는지까지 어긋나지 않는다.

### 예
가입할 때 이미 쓰는 아이디라고 막히는 그 순간, 어딘가에 쌓인 표를 확인하고 온 것이다.
...
```도해
층: 데이터베이스는 무엇 위에 무엇이 얹혀 있나
질의 언어 :: SQL 로 무엇을 원하는지만 적어 보낸다
질의 처리 :: 문장을 해석하고 어떻게 찾을지 정한다
저장 엔진 :: 표와 인덱스를 정해진 덩어리 단위로 읽고 쓴다
디스크 :: 파일이 실제로 놓이는 자리. 꺼져도 남는다
= 위는 무엇을 원하는지만 말하고, 아래로 갈수록 어디에 어떻게 놓였는지가 된다
```
...
**"자료를 어긋나지 않게 맡아 두는 그 자리"** 라고 읽으면 된다. 어긋나지 않게 지키는 방법이 갈래를 가르는데, 관계형은 표 사이의 관계와 트랜잭션으로 DB 가 직접 지키고, NoSQL 은 그 검사를 앱 쪽에 넘기는 대신 모양의 자유와 서버를 붙여 늘리는 확장을 가져간다.

## 🧒 열 살에게

책을 아무 데나 쌓아 두면 누가 무엇을 빌려 갔는지 아무도 몰라. 도서관은 책마다 자리를 정해 두고, 빌려 갈 때마다 누가 언제 가져갔는지를 한 곳에만 적어 둬서 여럿이 한꺼번에 빌리고 돌려줘도 그 책이 지금 어디 있는지는 늘 한 가지로 딱 정해져. 그런 도서관을 컴퓨터 안에 지어 놓은 거야.

## ❓ 이해했는지

- 송금 도중에 프로그램이 죽었는데도 돈이 사라지지 않는 이유는 → 해결하는 문제
- 인덱스를 다섯 개 걸었더니 저장이 느려졌다면 무엇 때문인가 → 작동 원리
- 사용자 이름이 바뀌었는데 주문 표를 하나도 안 고쳐도 되는 이유는 → 실제 사례
