...
### 예
긴 PDF 를 올려놓고 물었더니 답 아래 붙은 출처가 문서 전체가 아니라 한 문단을 가리킨다.
...
**"찾기 좋게 미리 잘라 두는 그 손질"** 이라고 읽으면 된다. 모델 입력 한도 때문에 자르기도 하지만, 더 큰 이유는 필요한 대목만 골라내기 위해서다. 자르는 기준과 겹침의 양이 검색 품질을 정한다.

## 🧒 열 살에게

두꺼운 책을 통째로 들고 다니면 무거운데, 장마다 쪽지를 붙여 두면 필요한 쪽만 펴 보면 되지? 긴 글도 미리 토막을 내서 토막마다 쪽지를 붙여 둬. 그래서 뭘 물으면 책 전부가 아니라 그 토막 하나만 꺼내 와.
