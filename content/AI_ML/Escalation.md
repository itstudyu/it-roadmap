...
### 이름
Escalate :: 위 단계로 올리다
= 에스컬레이터와 뿌리가 같은 말이다. 제자리에서 더 애쓰는 게 아니라 한 층 위로 올려 보낸다

### 비유
신입 상담원이 모르는 질문을 받으면 선배를 부르는 것. 억지로 답하는 쪽이 더 큰일을 만든다.

### 예
상담 챗봇 창에서 상담원 연결을 누르면 사람이 이어받는데, 그 넘어가는 자리가 여기다.

### 직접
상담 챗봇에 "상담원 연결" 이라고 쳐 보라. 자료를 더 뒤지지 않고 곧장 사람 쪽으로 넘어간다.
...
**"못 하겠다 싶으면 사람에게 올려 보내는 그 통로"** 라고 읽으면 된다. 확신도와 반복 실패와 사용자의 요청과 감정을 보고 판단하며, 넘긴 사유를 함께 전달해야 받는 쪽이 이어서 처리할 수 있다.

## 🧒 열 살에게

가게에서 아르바이트 형한테 어려운 걸 물으면 "잠시만요" 하고 사장님을 부르지? 아는 척 대충 대답했다간 더 큰일이 나니까 그래. 여기서도 답이 자신 없거나 네가 화가 나 있으면, 계속 붙잡고 있지 않고 바로 사람한테 넘겨.

## ❓ 이해했는지

- 사용자가 같은 질문을 표현만 바꿔 계속 묻는 상황에서 무엇이 빠져 있나 → 해결하는 문제
- 답을 정확히 알고 있는데도 넘겨야 하는 경우는 어떤 때인가 → 흔한 오해
- 티켓에 넘긴 사유를 적어야 하는 이유가 무엇인가 → 작동 원리
