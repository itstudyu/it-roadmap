...
User :: 그것을 쓸 사람
Story :: 겪는 이야기
= 붙여 읽으면 "쓸 사람이 겪는 이야기". 명세서가 아니라 이야기라고 부른 데 뜻이 있다

### 비유
식당 메뉴판. 손님이 무엇을 원하는지까지만 적혀 있고 어떻게 조리할지는 주방이 정한다.

### 예
앱 리뷰에 저장 기능을 넣어 달라고만 쓰는 사람과 지하철에서 읽으려고 그렇다고 쓰는 사람이 있다. 뒤쪽이 이 형식이다.

### 직접
아무 앱의 리뷰를 열어 무엇을 해 달라고만 쓴 글과 왜 필요한지까지 쓴 글을 하나씩 찾아 보라. 뒤쪽만 무엇을 만들지 정할 수 있다.
...
**"쓸 사람 입장에서 세 줄로 적은 그 이야기"** 라고 읽으면 된다. 세부를 다 담는 명세가 아니라 팀이 대화를 시작할 지점이고, 끝났는지는 인수 기준으로 따로 정한다. 한 스프린트에 들어가지 않을 만큼 크면 에픽이므로 더 쪼갠다.

## 🧒 열 살에게

식당에서 "맵지 않게 해 주세요"라고는 말해도 어느 냄비에 어떤 불로 익히라고까지는 안 하지? 원하는 것과 그 까닭까지만 말하면 주방이 제일 좋은 방법을 골라 줘. 처음부터 방법까지 시켜 버리면 더 나은 방법이 있어도 못 쓰게 돼.

## ❓ 이해했는지

- "개발자로서 캐시를 넣고 싶다"가 좋은 스토리가 아닌 이유는 → 흔한 오해
- 스토리에 버튼 위치까지 적으면 팀은 무엇을 잃게 되나 → 해결하는 문제
- 한 스프린트에 못 끝낼 만큼 큰 요구는 무엇이라 부르고 어떻게 다루나 → 구성
