...
Back :: 밀려 있는
Log :: 적어 둔 줄
= 붙여 읽으면 "밀려 있는 일을 적어 둔 줄". 쌓인 것을 버리지 않고 세워 둔다는 뜻이다
...
### 직접
메모 앱에 지금 안 해도 되는 일을 열 줄 적고 급한 순으로 다시 세워 보라. 아래 몇 줄은 이번 주에 손도 못 댄다는 것이 바로 보인다.
...
**"밀린 일을 한 줄로 세워 둔 그 목록"** 이라고 읽으면 된다. 무엇을 먼저 할지 정하는 판단이 한 자리로 모이니, 새 요청이 와도 다투는 대신 목록의 몇 번째에 놓을지만 정하면 된다. 그 순서는 상황에 따라 계속 다시 매긴다.

## 🧒 열 살에게

식당에 사람이 많으면 번호표를 뽑고 기다리지? 할 일도 그렇게 줄을 세워 두는데, 이 줄은 더 급한 게 오면 앞으로 끼어들 수 있어. 그래서 뒤쪽에 오래 서 있던 것은 차례가 끝내 안 오기도 해.
