...
Client :: 부탁하는 쪽
URL :: 가져올 주소 한 줄
= 붙여 읽으면 "주소 한 줄로 부탁하는 쪽". 받아 두는 도구가 아니라 거는 도구라는 게 이름에 있다

### 비유
전화 주문. 가게에 직접 가지 않고 번호를 눌러 필요한 것만 말한다.

### 예
개발 문서에서 그대로 복사해 붙여 넣으라고 알려주는 curl 로 시작하는 한 줄이 그것이다.

### 직접
터미널에서 `curl -i https://example.com` 을 쳐 보라. 브라우저가 감춰 두던 상태 코드와 헤더가 본문 위에 그대로 뜬다.
...
**"주소 한 줄로 문을 두드려 보는 그 명령"** 이라고 읽으면 된다. 요청 하나가 글자 한 줄이 되기 때문에 스크립트에 넣거나 남에게 그대로 보낼 수 있다. 브라우저가 알아서 해주던 리다이렉트나 쿠키는 옵션으로 직접 켜야 한다.

## 🧒 열 살에게

가게까지 안 가고 전화로 "이거 하나 주세요" 하면 되지? 컴퓨터에서도 한 줄만 적어서 저 멀리 있는 곳에 필요한 걸 달라고 말할 수 있어. 대신 전화라서 가게 안 구경은 못 하고, 달라고 한 것만 딱 돌아와.
