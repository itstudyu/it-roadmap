...
Control :: 누구 것인지 가림
Address :: 주소
= 붙여 읽으면 "한 선을 나눠 쓸 때 누구 것인지 가리는 주소". 그 선을 벗어나면 힘이 없다

### 비유
여권 번호. 사는 곳이 바뀌어도 번호는 그대로고, 번호만 알아서는 그 집을 찾아갈 수 없다.

### 예
공유기 관리 화면의 접속 기기 목록에 기기 이름과 함께 뜨는 여섯 토막짜리 번호가 이것이다.

### 직접
폰 설정에서 "정보" 를 열어 보라. "와이파이 주소" 라는 여섯 토막짜리 번호가 있고, 요즘 폰은 와이파이마다 이 번호를 바꿔 쓴다는 안내도 붙어 있다.
...
번호 없이 || MAC 주소로
받는 쪽 표시 :: 적을 값이 없다 || 48비트로 적는다
프레임 거르기 :: 전부 열어 봐야 || 자기 것만 올림
...
- **클라우드 가상 장비** — 애저는 NIC 이 VM 에 붙어 처음 켜질 때 MAC 을 배정한다. 어떤 값을 쓸지 고를 수 없고, NIC 을 지우기 전까지는 그대로 남는다.
...
**"자리가 아니라 장비에 붙는 그 번호"** 라고 읽으면 된다. 앞부분은 IEEE 가 조직에 나눠 주고 뒷부분은 그 조직이 붙이기 때문에 겹치지 않는다. 다만 이 번호가 힘을 쓰는 범위는 같은 망 안까지이고, 라우터를 지나면 새 번호로 갈린다.

## 🧒 열 살에게

여권에 적힌 번호는 이사를 가도 안 바뀌지? 대신 그 번호만 대고는 아무도 네 집을 찾아올 수 없어. 그 번호는 지금 같이 서 있는 줄 안에서 "이거 누구 거예요" 하고 주인을 가릴 때만 쓰여.
