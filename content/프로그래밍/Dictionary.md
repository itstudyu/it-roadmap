...
Dictionary :: 낱말 찾는 사전
Map :: 짝지어 이어 줌
= 사전에서 낱말을 대면 뜻이 나오듯, 이름을 대면 값이 나온다

### 비유
사물함. 번호가 아니라 이름이 붙어 있어서, 몇 번째 칸인지 세지 않고 이름만 보고 문을 연다.

### 예
연락처에서 이름을 쳐서 번호를 찾는 일이 이것이다. 그 사람이 몇 번째로 저장됐는지는 아무도 모른다.
...
**"이름표 보고 바로 여는 그 칸"** 이라고 읽으면 된다. 이름과 값을 짝지어 한 자료구조 안에 담기 때문에 위치를 맞출 일이 없고, 항목이 늘어도 찾는 속도가 거의 변하지 않는다.

## 🧒 열 살에게

신발장 칸에 번호 대신 이름표가 붙어 있는데, 이름 첫 글자마다 갈 자리가 미리 정해져 있다고 해 봐. "유" 로 시작하면 무조건 셋째 줄 다섯째 칸이니까, 이름표를 하나씩 읽어 볼 필요 없이 첫 글자만 보고 바로 그 칸으로 가. 칸이 백 개로 늘어나도 첫 글자를 보고 자리를 정하는 건 똑같으니까 걸리는 시간은 그대로야.

## ❓ 이해했는지

- 항목이 백만 개인 딕셔너리에서 값 하나 꺼내는 속도가 열 개일 때와 비슷한 이유는 → 그림
- 리스트를 키로 쓰면 대부분의 언어가 막는 이유는 → 흔한 오해
- 있을지 없을지 모르는 키를 꺼낼 때 대괄호를 쓰면 무슨 일이 일어나나 → 작동 원리
