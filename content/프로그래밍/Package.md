...
Package :: 묶어 싼 꾸러미
Module :: 갈아 끼우는 한 칸
= 낱개 한 칸이 모듈, 그 칸들을 싸서 이름을 붙인 꾸러미가 패키지다
...
**"이름표 붙여 묶어 둔 그 바인더"** 라고 읽으면 된다. 나누는 이유는 이름이 겹치는 걸 막고 필요한 조각만 가져오기 위해서다. 폴더와 파일 이름이 그대로 부를 때의 앞자리가 된다.

## 🧒 열 살에게

종이를 한 장씩 책상에 흩어 두면 제목이 같은 종이가 두 장일 때 헷갈리지? 그래서 붙어 다니는 종이끼리 바인더에 모으고 겉에 이름을 써 붙여. 그러면 제목이 같아도 어느 바인더에서 꺼낸 종이인지로 갈리고, 친구한테 줄 때도 바인더째 건네면 돼.
